...
$query_rs_products = sprintf("SELECT id_products,id_member,product_name,product_name_EN, product_image_illustrate, product_day_update FROM products WHERE id_member = %s AND product_type='buy' ORDER BY product_day_update DESC", GetSQLValueString($session_member_rs_products, "int"));
...
<?php if ($totalRows_rs_products > 0) { // Show if recordset not empty ?>
  <tr>
    ...
    <td><a href="index.php?mod=fb&amp;id_products=<?php echo $row_rs_products['id_products'];?>"><?php echo sua; ?></a> | <a  href="index.php?mod=delb&amp;ip=<?php echo $row_rs_products['id_products'];?>" onclick="if(!window.confirm('<?php echo bancomuonxoasanphamnay;?>')){return false;};"><?php echo xoa;?></a></td>
  </tr>
