...
$key3_rs_search = "-1";
if (isset($_GET['key3'])) {
  $key3_rs_search = $_GET['key3'];
}
mysql_select_db($database_ketnoi, $ketnoi);
$query_rs_search = sprintf("SELECT id_products, p.id_country,p. id_member, product_name, product_short_describe, product_image_illustrate,mb.member_kind,ct.country_name,country_image_illustrate,cp.company_name FROM products AS p,member AS mb,country AS ct,company AS cp WHERE product_type=%s AND p.id_country=%s AND (LOWER(CONVERT(product_name USING utf8)) LIKE %s OR LOWER(CONVERT(product_short_describe USING utf8)) LIKE %s  OR LOWER(CONVERT(product_name USING utf8)) LIKE %s OR LOWER(CONVERT(product_short_describe USING utf8)) LIKE %s) AND product_visible =1 AND p.id_member=mb.id_member AND mb.member_id_country=ct.id_country AND mb.id_member=cp.company_id_member", GetSQLValueString($key2_rs_search, "text"),GetSQLValueString($key3_rs_search, "int"),GetSQLValueString("%" . $key1_rs_search . "%", "text"),GetSQLValueString("%" . $key1_rs_search . "%", "text"),GetSQLValueString("%" . $key1_khongdau_rs_search . "%", "text"),GetSQLValueString("%" . $key1_khongdau_rs_search . "%", "text"));
            	...
            	<?php do { ?>
              	    ...
              	    <div class="box_2">
              	      <div class="img"> <img src="<?php echo $objDynamicThumb1->Execute(); ?>" border="0" align="left" />
              	        <h2><a href="#"><?php echo search_highlight($searchstring,"<span style='background-color:yellow'><b>$searchstring</b></span>",$row_rs_search['product_name']); ?></a></h2>
                          <label><?php echo $row_rs_search['product_short_describe']; ?></label>
           	          </div>
                        <div class="c-info">
