...
$query_rs_products_sell = "SELECT id_products,id_procat, products.id_member, product_name, product_day_update FROM products,member WHERE product_visible = 1 AND product_type='sell' AND products.id_member=member.id_member ORDER BY product_day_update DESC";
...
<script type="text/javascript" src="library/js/slide_buy_sell.js"></script>
<script type="text/javascript" src="library/js/slide_buy_sell_index.js"></script>
<!-- offer list -->
...
<div id="hots">
	<div id="mid_info_buy">
            <ul id="list_buying_leads" class="hot_offer_list">
           <?php do { 
...
$totalRows_rs_member_sell = mysql_num_rows($rs_member_sell);
			...
			?>
              <li>
                <div class="date"><?php echo date('d-m-Y',strtotime($row_rs_products_sell['product_day_update'])); ?></div>
                <img src="uploads/country_flag/<?php echo $row_rs_member_sell['country_image_illustrate']; ?>" width="12px" height="12px"  align="left" border="0"/>&nbsp;<a href="shop/<?php echo member_VIP($row_rs_member_sell['member_kind']);?>/index.php?mod=detail&amp;imb=<?php echo $row_rs_products_sell['id_member'];?>&amp;ipc=<?php echo $row_rs_products_sell['id_procat'];?>&amp;ip=<?php echo $row_rs_products_sell['id_products'];?>" title="<?php echo $row_rs_products_sell['product_name']; ?>" target="_blank"><?php echo getstr($row_rs_products_sell['product_name'],35); ?></a></li> 
                 
    	  <?php } while ($row_rs_products_sell = mysql_fetch_assoc($rs_products_sell)); ?>  
            </ul>
	</div>
        <script type="text/javascript">
<!--
var hot_selling_leads = EC.widget.SimpleScroll.decorate('list_buying_leads', {lingHeight:20, startDelay: 1});
-->
</script>
 	<div id="mid_info_sell">
            <ul id="list_selling_leads" class="hot_offer_list">
             <?php do { 
	  ////truy van lay hinh anh co cua thanh vien thuoc san pham chao mua
...
$totalRows_rs_member_buy = mysql_num_rows($rs_member_buy);	
	  ?>
              <li>
                <div class="date"><?php echo date('d-m-Y',strtotime($row_rs_products_buy['product_day_update'])); ?></div>
                <img src="uploads/country_flag/<?php echo $row_rs_member_buy['country_image_illustrate']; ?>" width="12px" height="12px" align="left" border="0"/>&nbsp;<a href="shop/<?php echo member_VIP($row_rs_member_buy['member_kind']);?>/index.php?mod=detail&amp;imb=<?php echo $row_rs_products_buy['id_member'];?>&amp;ipc=<?php echo $row_rs_products_buy['id_procat'];?>&amp;ip=<?php echo $row_rs_products_buy['id_products'];?>" title="<?php echo $row_rs_products_buy['product_name']; ?>" target="_blank"><?php echo getstr($row_rs_products_buy['product_name'],35); ?></a></li>   
 <?php } while ($row_rs_products_buy = mysql_fetch_assoc($rs_products_buy)); ?>
            </ul>
	</div>
      </div>
      <script type="text/javascript">
<!--
var hot_selling_leads = EC.widget.SimpleScroll.decorate('list_selling_leads', {lingHeight:20, startDelay: 2});
-->
</script>
</div>
