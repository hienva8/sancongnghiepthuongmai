<?php 
$query_rs_ad = "SELECT id_advertise,title,notes,notes_EN,file_image,fileflash,url,width,height,position FROM advertise WHERE (date_format(day_end,'%y%m%d') >= date_format(now(),'%y%m%d')) AND position='business' AND visible=1 Order By sort ASC";
$rs_ad = mysql_query($query_rs_ad,$ketnoi) or die(mysql_error());
$row_rs_ad = mysql_fetch_assoc($rs_ad);
$totalRows_rs_ad = mysql_num_rows($rs_ad);
?>
