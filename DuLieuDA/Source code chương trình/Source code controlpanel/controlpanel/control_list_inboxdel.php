...
$query_rs_inbox = sprintf("SELECT id_inbox, box_contact_department, inbox_sender, inbox_subject, inbox_day_update, inbox_recycle_bin,name FROM inbox,contact_department WHERE inbox.box_contact_department= contact_department.id_contact_department AND inbox_recipient = %s AND inbox_recycle_bin=1 ORDER BY inbox_day_update DESC", GetSQLValueString($session_member_rs_inbox, "int"));
...
?>
  <tr>
    ...
    <td><a href="index.php?mod=ibx_detail&amp;iibx=<?php echo $row_rs_inbox['id_inbox']; ?>"><?php echo xem; ?></a> | <a  href="index.php?mod=ibxdel&amp;iibx=<?php echo $row_rs_inbox['id_inbox'];?>" onclick="if(!window.confirm('<?php echo bancomuonxoasanphamnay;?>')){return false;};"><?php echo xoa;?></a></td>
  </tr>
...
<?php if ($totalRows_rs_inbox == 0) { // Show if recordset empty ?>
  <tr>
    <td colspan="6" align="center"><?php echo hienchuacothudaxoa;?></td>
  </tr>
