...
$query_rs_products = "SELECT id_services,sv.id_services_categories,sv.id_member,sv.image_illustrate,sv.title,sv.short_describe,mb.member_kind,ct.country_name,ct.country_name_EN,ct.country_image_illustrate,cp.company_name FROM services AS sv,member AS mb, country AS ct,company AS cp WHERE sv.id_member=mb.id_member AND mb.member_id_country=ct.id_country AND mb.id_member=cp.company_id_member";
...
<h2><a href="../../shop/<?php  echo member_VIP($row_rs_products['member_kind']);?>/index.php?mod=svdetail&imb=<?php echo $row_rs_products["id_member"];?>&isv=<?php echo $row_rs_products['id_services'];?>&amp;isvc=<?php echo $row_rs_products['id_services_categories']; ?>"><?php echo $row_rs_products['title'];?></a></h2>
