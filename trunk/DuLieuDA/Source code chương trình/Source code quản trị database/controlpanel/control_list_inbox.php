...
$query_rs_inbox = sprintf("SELECT id_inbox, box_contact_department, inbox_sender, inbox_subject, inbox_day_update, inbox_recycle_bin,name FROM inbox,contact_department WHERE inbox.box_contact_department= contact_department.id_contact_department AND inbox_recycle_bin!=1 AND inbox_recipient = %s ORDER BY inbox_day_update DESC", GetSQLValueString($session_member_rs_inbox, "int"));
...
if (isset($row_rs_inbox['inbox_sender'])) {
  $sender_rs_company = $row_rs_inbox['inbox_sender'];
}
mysql_select_db($database_ketnoi, $ketnoi);
$query_rs_company = sprintf("SELECT company_name, company_name_EN FROM company WHERE company_id_member = %s", GetSQLValueString($sender_rs_company, "int"));
...
?>
  <tr>
    <td><?php echo $i; ?></td>
    <td><?php echo getstr($row_rs_company['company_name'],20); ?></td>
    ...
    <td><a href="index.php?mod=ibx_detail&amp;iibx=<?php echo $row_rs_inbox['id_inbox']; ?>"><?php echo xem; ?></a> | <a  href="<?php echo $linkcurrent;?>?mod=libx"  onclick="ajaxLoad('control_inbox_del.php?mod=libx&iibx=<?php echo $row_rs_inbox['id_inbox']; ?>','hienthi');"><?php echo xoa;?></a></td>
  </tr>
...
<?php if ($totalRows_rs_inbox == 0) { // Show if recordset empty ?>
  <tr>
    <td colspan="6" align="center"><?php echo hienchuacothuden;?></td>
  </tr>
